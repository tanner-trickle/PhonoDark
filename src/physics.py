...
def calc_binned_rate(mass, vE_vec, inputs, q_list, G_list, jacob_list, W_tensor, 
        ph_omega, ph_eigenvectors):
    """
        Computes the scattering rate binned in energy deposition for a given mass and Earth velocity vector.
    """

    n_E_bins = inputs['numerics']['numerics']['n_E_bins']
    num_atoms = inputs['material']['phonopy_config_info']['num_atoms']

    n_a = inputs['numerics']['numerics']['n_a']
    n_b = inputs['numerics']['numerics']['n_b']
    n_c = inputs['numerics']['numerics']['n_c']

    m_cell = np.sum(inputs['material']['phonopy_config_info']['atom_masses'])

    db_rate = np.zeros((n_E_bins))

    threshold = inputs['physics_model']['physics_parameters']['threshold']
    energy_bin_width = inputs['numerics']['numerics']['energy_bin_width']

    spin = inputs['physics_model']['dm_properties']['spin']

    m_med_input = inputs['physics_model']['physics_parameters']['m_med']

    # Check mediator mass
    if m_med_input == 'light':
        m_med = 0
    elif m_med_input == 'heavy':
        m_med = 10**12 # TeV
    else:
        m_med = m_med_input

    for q, q_vec in enumerate(q_list):

        q_mag = np.linalg.norm(q_vec)

        F_prop_val = ( q_mag**2 + m_med**2 )**(-1)

        c_dict_full = c_dict_form_full(q_vec, 
                inputs['material']['phonopy_config_info']['dielectric'], 
                inputs['physics_model']['c_dict'], 
                inputs['physics_model']['c_dict_form'],
                mass, 
                spin)
        ...
        if inputs['physics_model']['physics_parameters']['born_cor']:
            ...
            # cNe^2 terms, to be multiplied by velocity integrals g0, g1, g2
            
            cNe_sq_0 = (cNe00*np.conj(cNe00)
                            + ((spin*(spin+1))/3.0)*(
                                np.dot(cNe01, np.conj(cNe01))
                            )
                        )
                        
            cNe_sq_1 = ((spin*(spin+1))/3.0)*(
                                    np.matmul(np.conj(cNe11), cNe01) +
                                    np.matmul(cNe11, np.conj(cNe01))
                                    )
            
            cNe_sq_2 = ((spin*(spin+1))/3.0)*np.matmul(np.conj(cNe11), cNe11.T)

        for nu in range(inputs['material']['phonopy_config_info']['num_modes']):
            ...
            if energy_diff >= threshold:
                ...
                if v_minus_val < const.VESC:
                    ...
                    # clamp the bin number
                    bin_num = min( max( 0, math.floor((energy_diff-threshold)/energy_bin_width) ), n_E_bins - 1 )
                    
                    dw_val = np.zeros(num_atoms, dtype=complex)
                    pos_phase = np.zeros(num_atoms, dtype=complex)
                    
                    q_dot_e = np.zeros(num_atoms, dtype=complex)
                    q_dot_ZminusQ_dot_e = np.zeros(num_atoms, dtype=complex)
                    
                    VxcNe_0 = np.zeros(num_atoms, dtype=complex)
                    VxcNe_1 = np.zeros((num_atoms, 3), dtype=complex)
                    VxcNe_2 = np.zeros((num_atoms,3,3), dtype=complex)

                    prefactor = 0.5*(const.RHO_DM/mass)\
                                *(1.0/m_cell)*(2*const.PI)**(-3)*jacob_list[q]\
                                *(1.0/(n_a*n_b*n_c))*(1.0/energy_diff)\
                                *F_prop_val**2*const.KG_YR
                    
                    for j in range(num_atoms):
                        
                        dw_val[j] = np.dot(q_vec, np.matmul(W_tensor[j], q_vec))
                        pos_phase[j] = (1j)*np.dot(G_list[q], inputs['material']['phonopy_config_info']['eq_positions_XYZ'][j])

                        q_dot_e[j] = np.dot(q_vec, ph_eigenvectors[q][nu][j])
                        
                        if inputs['physics_model']['physics_parameters']['born_cor']:
                            q_dot_ZminusQ_dot_e[j] = np.matmul(q_vec, np.matmul(inputs['material']['phonopy_config_info']['born'][j], ph_eigenvectors[q][nu][j]))\
                                                        - (inputs['material']['properties']['N_list']['p'][j]-inputs['material']['properties']['N_list']['e'][j])*q_dot_e[j]

                            # cross terms between V_j^* and cNe, to be multiplied by velocity integrals g0, g1, g2
                            
                            VxcNe_0[j] = (
                                    cNe00*np.conj(total_V_func_eval["00"][j])
                                    + ((spin*(spin+1))/3.0)*(
                                            np.dot(cNe01, np.conj(total_V_func_eval["01"][j]))
                                        )
                                    )
                            
                            VxcNe_1[j] = (
                                            cNe00*np.conj(total_V_func_eval["10"][j])
                                            + ((spin*(spin+1))/3.0)*(
                                                np.matmul(np.conj(total_V_func_eval["11"][j]), cNe01) +
                                                np.matmul(cNe11, np.conj(total_V_func_eval["01"][j]))
                                                )
                                        )
                                        
                            VxcNe_2[j] = ((spin*(spin+1))/3.0)*np.matmul(np.conj(total_V_func_eval["11"][j]), cNe11.T)
                    ...
                    for j in range(num_atoms):
                        ...
                        for jp in range(num_atoms):
                            ...
                            if inputs['physics_model']['physics_parameters']['born_cor']:
                                ...
                                g0_rate = g0_val*VxcNe_0[jp]
                                g1_rate = np.dot(g1_vec, VxcNe_1[jp])
                                g2_rate = np.trace(np.matmul(g2_tens, VxcNe_2[jp]))
                                
                                delta_rate -= (q_dot_ZminusQ_dot_e_star_j*q_dot_e_jp)*(g0_rate + g1_rate + g2_rate)
                                
                                
                                g0_rate = g0_val*np.conj(VxcNe_0[j])
                                g1_rate = np.dot(g1_vec, np.conj(VxcNe_1[j]))
                                g2_rate = np.trace(np.matmul(g2_tens, np.conj(VxcNe_2[j])))

                                delta_rate -= (q_dot_e_star_j*q_dot_ZminusQ_dot_e_jp)*(g0_rate + g1_rate + g2_rate)
                                
                                g0_rate = g0_val*cNe_sq_0
                                g1_rate = np.dot(g1_vec, cNe_sq_1)
                                g2_rate = np.trace(np.matmul(g2_tens, cNe_sq_2))

                                delta_rate += (q_dot_ZminusQ_dot_e_star_j*q_dot_ZminusQ_dot_e_jp)*(g0_rate + g1_rate + g2_rate)

                            delta_rate *= prefactor*(inputs['material']['phonopy_config_info']['atom_masses'][j]*inputs['material']['phonopy_config_info']['atom_masses'][jp])**(-0.5)*np.exp(exp_val)

                            db_rate[bin_num] += np.real(delta_rate)
...
#                     
#                     VxcNe_0 = np.zeros(phonopy_params['num_atoms'], dtype=complex)
#                     VxcNe_1 = np.zeros((phonopy_params['num_atoms'],3), dtype=complex)
#                     VxcNe_2 = np.zeros((phonopy_params['num_atoms'],3,3), dtype=complex)
#                     
#                     prefactor = 0.5*(const.RHO_DM/mass)\
#                                 *(1.0/m_cell)*(2*const.PI)**(-3)*jacob_list[q]\
#                                 *(1.0/(n_a*n_b*n_c))*(1.0/energy_diff)\
